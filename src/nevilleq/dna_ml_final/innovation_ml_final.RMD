...
mod.list <- list(
  ...
  rf  = train(innovation ~ ., data = train.df, method = "rf", family = "bernoulli",
             trControl = trainControl(method = "cv", number = 5)),
  gbm = train(innovation ~ ., data = train.df, distribution = "bernoulli", method = "gbm",
             trControl = trainControl(method = "cv", number = 5)),
  #svm = ,
  #nn  = ,
...
ggsave("./src/nevilleq/dna_ml_final/innovation_ml_figures/19_8_1_inn_roc.jpg", roc.gg)
```
...
```{r echo = FALSE, include = FALSE}
ggsave("./src/nevilleq/dna_ml_final/innovation_ml_figures/19_8_1_inn_cost.jpg", cost.gg)
```
...
boot_diagnostics <- function(i) {
  ...
  mod.list <- list(
  glm = train(innovation ~ ., data = sample.df[boot.resamps[[i]], ], method = "glmnet", family = "binomial",
             trControl = trainControl(method = "cv", number = 5)),
  rf  = train(innovation ~ ., data = sample.df[boot.resamps[[i]], ], method = "rf", family = "bernoulli",
             trControl = trainControl(method = "cv", number = 5)),
  gbm = train(innovation ~ ., data = sample.df[boot.resamps[[i]], ], distribution = "bernoulli", method = "gbm",
             trControl = trainControl(method = "cv", number = 5))
  #svm = ,
  #nn  = ,
...
#                                 optimize_alpha = TRUE)
  ...
  preds = c(map(mod.list, ~predict(.x, newdata = sample.df[out.of.bag[[i]], ], type = "prob")[ ,2]), lda_predict(mod.list[[4]], lda.sample[out.of.bag[[i]], ], sample.df[boot.resamps[[i]], ]))
  )
