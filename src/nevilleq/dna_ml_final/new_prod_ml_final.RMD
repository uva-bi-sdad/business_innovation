...
lda_predict <- function(lda, new.data) {
    #find which cluster has more postive response
    cluster <- ifelse(lda$theta[,2] > 0.5, 2, 1)
    cluster <- which.max(c(mean((cluster == 1) == new.data[ ,1]),
                          mean((cluster == 2) == new.data[ ,1])))
    #Return Predicted Probability of positive response
    list(lda = predict(lda, new.data, k = 2, method = "gibbs", iterations = iter, burnin = burn)[ ,cluster])
      }
...
accuracy <- function(preds, threshold) {
  
  mean((preds > threshold) == test.df[,1])

  }

precision <- function(preds, threshold) {
  
  preds     <- preds > threshold
  true.pos  <- sum(preds & test.df[,1])
  false.pos <- sum(preds & !test.df[,1])
  return(true.pos / (true.pos + false.pos))

}

recall <- function(preds, threshold) {
  
  preds     <- preds > threshold
  true.pos  <- sum(preds & test.df[,1])
  false.neg <- sum(!preds & test.df[,1])
...
boot_diagnostics <- function(i) {
  ...
  fit   = mod.list,
  type  = c(rep("Supervised", 3), "Unsupervised"),
  preds = c(map(mod.list[-4], ~predict(.x, newdata = sample.df[out.of.bag[[i]], ], type = "prob")[ ,2]), lda_predict(mod.list[[4]], lda.sample[out.of.bag[[i]], ]))
  )
...
return(opt.result %>% dplyr::select(model, accuracy:recall, auc))
...
result.glmnet <- foreach(i = 1:1000, .combine = combine, .multicombine = TRUE,
  ...
  outSub <- taskFun(hdd.df, n.boot = 1)
  list(coefficients = outSub$coefficients, lambda = outSub$lambda, boot.selected = outSub$boot.selected, alpha = outSub$alpha)
} 
result.glmnet[[3]] <- result.glmnet[[3]] %>% t()
