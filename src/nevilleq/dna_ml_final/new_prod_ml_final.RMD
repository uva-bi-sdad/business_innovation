...
lda_predict <- function(lda, lda.data, new.data) {
    #find which cluster has more postive response
    cluster <- ifelse(lda$theta[,2] > 0.5, 2, 1)
    cluster <- which.max(c(mean((cluster == 1) == new.data$subject_code_ns),
                          mean((cluster == 2) == new.data$subject_code_ns)))
    #Return Predicted Probability of positive response
    list(lda = predict(lda, lda.data, k = 2, method = "gibbs", iterations = iter, burnin = burn)[ ,cluster])
      }
...
pred.df <- tibble(
  ...
  preds = c(map(mod.list[-4], ~predict(.x, newdata = test.df, type = "prob")[ ,2]), lda_predict(mod.list[[4]], test.lda))
)
```
...
roc.gg <- pred.df %>%
  mutate(
    roc = map(preds, ~roc.log(test.df, .x)),
    auc = map_dbl(roc, auc_func),
    model = as.factor(model) %>% fct_recode("LDA" = "lda", "GLM" = "glm", "Random Forest" = "rf", "Gradient Boosting" = "gbm") %>% fct_reorder(., auc)
  ) %>%
  unnest(roc) %>%
...
ggsave("./src/nevilleq/dna_ml_final/nps_ml_figures/19_7_31_nps_roc.jpg", roc.gg)
```
...
cost.gg <- pred.df %>%
  ...
  unnest(cost) %>%
  mutate(model = fct_recode(model, "LDA" = "lda", "GLM" = "glm", "Random Forest" = "rf", "Gradient Boosting" = "gbm") %>% fct_reorder(cost, min)) %>%
...
```{r echo = FALSE, include = FALSE}
ggsave("./src/nevilleq/dna_ml_final/nps_ml_figures/19_7_31_nps_cost.jpg", cost.gg)
```
...
accuracy <- function(preds, threshold, test.df = test.df) {
  
  mean((preds > threshold) == test.df$subject_code_ns)

  }

precision <- function(preds, threshold, test.df = test.df) {
...
recall <- function(preds, threshold, test.df = test.df) {
...
#                                 optimize_alpha = TRUE)
  ...
  preds = map(mod.list, ~predict(.x, newdata = sample.df[out.of.bag[[i]], ], type = "prob")[ ,2])#, lda_predict(mod.list[[4]], lda.sample[out.of.bag[[i]], ], sample.df[out.of.bag[[i]], ]))
  )
...
return(opt.result %>% dplyr::select(iteration, roc, auc, cost, min_cost_threshold, preds, model, accuracy:recall, auc))
...
result.glmnet <- foreach(i = 1:10, .combine = rbind, .packages = c("tidyverse", "glmnet", "caret", "elasticnet")) %dopar% {
  outSub <- taskFun(i)
  outSub
} 

write_rds(result.glmnet, "./data/src/nevilleq/dna_ml_final/nps_result/19_7_31_nps_boot_result.RDS")
