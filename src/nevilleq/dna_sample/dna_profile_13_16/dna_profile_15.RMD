...
#Size
num.nps <- map_lgl(.x = dna.3.dt$subject_codes_split, ~ c("c22") %in% .x) %>% sum()

N <- 1000
#n <- (1/2 * N)  %>% ceiling()
#n <- (1/5 * N)  %>% ceiling()
#n <- (1/10 * N) %>% ceiling()
n <- (num.nps/nrow(dna.3.dt) * N) %>% ceiling() 

#Sample
set.seed(2019)
sample <- c(sample(which(dna.3.dt$subject_code_ns == TRUE), n),
            sample(which(dna.3.dt$subject_code_ns == FALSE), 1000 - n)
            )
...
#Write out RDS
write_rds(sample.dt, "./data/working/DNA_Aggregated/Machine_learning_sample/2019_7_23_prop_sample.RDS")
