...
train.split <- 80
set.seed(2019)
sample <- sample(1:1000, train.split, replace = FALSE)
...
sample.df <- list.files(path = file.path) %>%
  ...
  mutate(
    ...
    train       = map(.x = data, ~.x %>% slice(sample)),
    test        = map(.x = data, ~.x %>% slice(-sample))
  ) %>%
...
sample.df <- sample.df %>%
  ...
  mutate(
    glm = map(.x = data, ~train(train.product ~ ., data = data.frame(train.product, train.df), method = "glmnet",
