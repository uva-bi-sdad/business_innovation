...
kmeans.df <- sample.df %>%
  ...
  mutate(
    kmean     = map(.x = train, ~kmeans(.x %>% dplyr::select(-subject_code_ns), centers = 2)),
    ...
    k_preds    = map2(.x = kmean, 
                      ...
                      ~predict.kmeans(.x, 
                                      .y %>% 
                                        ...
                                        as.matrix(), 
                                      method = "classes")),
    k_accuracy = map_dbl(.x = k_preds, .y = cluster, ~mean(.x == .y))
  )
...
accuracy <- hclust.df %>%
            dplyr::select(sample, train, test, hclust_average:hclust_centroid) %>%
            gather(key = method, value = model) %>%
            mutate(
              cluster   = map2_dbl(.x = train, 
                     .y = model, 
                     ~which.max(
