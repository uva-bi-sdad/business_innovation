...
files_10k_yrs <- plyr::ddply(files_10k, "V1", summarise,
               num_yrs = length(V3))
...
tmp = c(" Stuff Adderall&#174;", "Other Stuff >Adderall XL&#174;")

str_extract_all(tmp, "(?<=\\s)(.*?[^\\s].{2}|[^\\s]*?.{3})(?=&#174;)")
str_extract_all(tmp, "(.[^A-Z]*?.{3})(?=&#174;)")
...
#f <-
find_occurences(ciks_10k[2], 2014, "10-K", "(?<=\\s)([^\\s]*?.{3})(?=&#174;)")
tmp = find_occurences2(ciks_10k[13], 2014, "10-K", "(?<=\\s)([^\\s]*?.{3})(?=&#174;)")
find_occurences2 <- function(cik, yr, frm, regex) {
