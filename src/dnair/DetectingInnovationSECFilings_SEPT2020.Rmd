...
```{r ingestion_counts_2}
knitr::kable(data.frame("Total Companies" = c(length(refciks)), 
                        "Total Listings" = c(length(ndc_file_names)),
                        "Total Years" = c(length(unique(str_extract(ndc_paths, "\\d{4}$"))))
                        ))
```
...
```{r read_fda_1}
fda <- readxl::read_excel(paste0(project_folder, "original/fda_drugs/Copy of FDA Database - COMBINED V2.xlsx"), sheet = "Sheet1")
...
[[MISSING]] - link to where the FDA data came from originally, drugs @ fda maybe here
[[is this it?]](https://www.fda.gov/drugs/drug-approvals-and-databases/drugsfda-data-files)

```{r read_fda_2}
fda <- fda %>% mutate(sub_type = stringr::str_extract(submission, "ORIG|SUPPL"),
                      sub_class = recode(submission_classification_, 
...
fda <- fda %>% filter(submission_status == "Approval")
```
...
```{r plot_fda_2}
ggplot(data = fda, aes(x = as.integer(lubridate::year(approval_date)), fill = sub_type)) +
...
```{r ndc_vs_fda_1}
fda_compare <- fda %>% count(submission_status, lubridate::year(approval_date), sub_type,  sub_class_simple, sub_class)
...
```{r, eval = FALSE}
fda %>% filter(as.numeric(lubridate::year(approval_date))  == 2013) %>% filter(str_detect(str_to_lower(drug_name), "glipizide"))
fda %>% filter(str_detect(str_to_lower(drug_name), "glipizide")) %>% arrange(approval_date)
```
...
```{r}
url <- "https://www.sec.gov/Archives/edgar/data/1003642/000143774913001948/0001437749-13-001948.txt"
filing_path <-  ndc_file_names[1]
```

![Sample Filing Header](DetectingInnovationSECFilings_files/figure-html/CopyOfsec_writeup/SEC_FILING_HEADER_HTML.png)

![Sample Filing Header](DetectingInnovationSECFilings_files/figure-html/CopyOfsec_writeup/SEC_FILING_HEADER_TExT.png)

First the text of all filings was ingested, such that each filing was represented by a single text file that contains the filing as well as all of its supplemental materials. Each filing represents an observation of the dataset. An example SEC filing can be found here:

    * [html url](https://www.sec.gov/Archives/edgar/data/1003642/000143774913001948/impax_10k-123112.htm)
    * [txt url](https://www.sec.gov/Archives/edgar/data/1003642/000143774913001948/0001437749-13-001948.txt)

The process of extracting text from the filings was constructed iteratively and resulted in the following steps. First, to facilitate text extraction for each filing, non-readable attachments, most often appearing in binary, (graphics, zip files, and excel documents) were removed. Next, a custom process for handling string encoding of the text documents was devised and applied. These steps included:

    * replacement of carrot (e.g., <>) symbol string encodings with the actual symbol
    * replacement of registered and trademarked (e.g., ® ™) symbol string encodings with the actual symbol
    * replacement of custom XML for tables and reports 

These steps facilitated reading and extraction of text nodes in XML; due to the non-standardized use of Extensible Markup Language (XML) across filings, the raw filings needed these further pre-processing steps. 
...
```{r}
data.frame(get_context_pos(word_vec = test, target = "melafind", n = 1, p_o_s = "Adjective"))  %>% knitr::kable()
