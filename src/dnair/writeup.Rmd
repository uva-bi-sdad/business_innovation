...
```{r}
origcomp_details$Industry <- origcomp_details$`SIC Industry`
compcount1 <- origcomp_details %>% 
    group_by(`SIC Industry`) %>%
...
To the extent possible, we want to remove any cases where the non-English word used is actually a company. So we use our list of companies in the SEC database, the original set of `r length(orig_companies) ` companies. 
