...
ggplot(data = matchsets %>% group_by(success) %>% summarise(n = sum(n)), aes(x = "", y = n, fill = success)) +
  ...
  geom_text(stat = "identity", aes(label = n, y = n - 1000), size = 7) +
  coord_polar(theta = "y", start = pi / 3) + 
  customPlot2 +
  #scale_fill_manual(values = categorical12[6:8], na.translate = TRUE) +
  labs(title = "Number of captures per capture method",
...
ggplot(valunval_ct_family__, aes(x = reorder(family, -n), y = n, fill = val_status)) + 
  geom_bar(stat = "identity") + 
  coord_flip()
...
total_unval <- final_product_list_singlefirstmentionsONLY %>% 
...
total_val <- final_product_list_singlefirstmentionsONLY_valunval %>% 
...
midndcportfolios <- doublebars %>% filter(sec == "ndc" & n < 100 & n > 10) %>% .$family
smallndcportfolios <- doublebars %>% filter(sec == "ndc" & n < 10 & n > 1) %>% .$family
onehitwonderndcportfolios <- doublebars %>% filter(sec == "ndc" & n == 1) %>% .$family

doublebars_topndc <- doublebars %>% filter(family %in% topndcportfolios)
doublebars_midndc <- doublebars %>% filter(family %in% midndcportfolios)
doublebars_smallndc <- doublebars %>% filter(family %in% smallndcportfolios)
doublebars_onendc <- doublebars %>% filter(family %in% onehitwonderndcportfolios)
...
doublebars_perc_mid <- doublebars_perc %>% filter(family %in% midndcportfolios)
doublebars_perc_small <- doublebars_perc %>% filter(family %in% smallndcportfolios)
doublebars_perc_onehit <- doublebars_perc %>% filter(family %in% onehitwonderndcportfolios)
...
## Big Portolio companies - over 100 listings

```{r}
ggplot(doublebars_topndc, aes(x=reorder(family, n))) + #reorder(day, -perc)
  geom_bar(data = subset(doublebars_topndc, sec != "unval"), aes(y=-n, fill = sec), stat = 'identity') +
  geom_bar(data = subset(doublebars_topndc, sec != "ndc"), aes(y=n, fill = sec), stat = "identity") +
  geom_text(data = doublebars_perc_top, aes(reorder(family, ndc), y = -140), label = doublebars_perc_top$ndc_perc) + 
  geom_text(data = doublebars_perc_top, aes(reorder(family, ndc), y = +150), label = doublebars_perc_top$val_perc) + 
  scale_y_continuous(labels = comma) +
  xlab("") +
  ylab("SEC Captures & Proprietary Validations vs  NDC PROPRIETARY Totals") +
  # scale_fill_manual(values = myCol6) +
...
## Mid Portolio companies - 10-100 listings

```{r}
ggplot(doublebars_midndc, aes(x=reorder(family, n))) + #reorder(day, -perc)
  geom_bar(data = subset(doublebars_midndc, sec != "unval"), aes(y=-n, fill = sec), stat = 'identity') +
  geom_bar(data = subset(doublebars_midndc, sec != "ndc"), aes(y=n, fill = sec), stat = "identity") +
  geom_text(data = doublebars_perc_mid, aes(reorder(family, ndc), y = -140), label = doublebars_perc_mid$ndc_perc) + 
  geom_text(data = doublebars_perc_mid, aes(reorder(family, ndc), y = +150), label = doublebars_perc_mid$val_perc) + 
  scale_y_continuous(labels = comma) +
  xlab("") +
  ylab("SEC Captures & Proprietary Validations vs  NDC PROPRIETARY Totals") +
  # scale_fill_manual(values = myCol6) +
...
## Small Portolio companies - 1-10 listings

```{r}
ggplot(doublebars_smallndc, aes(x=reorder(family, n))) + #reorder(day, -perc)
  geom_bar(data = subset(doublebars_smallndc, sec != "unval"), aes(y=-n, fill = sec), stat = 'identity') +
  geom_bar(data = subset(doublebars_smallndc, sec != "ndc"), aes(y=n, fill = sec), stat = "identity") +
  geom_text(data = doublebars_perc_small, aes(reorder(family, ndc), y = -140), label = doublebars_perc_small$ndc_perc) + 
  geom_text(data = doublebars_perc_small, aes(reorder(family, ndc), y = +150), label = doublebars_perc_small$val_perc) + 
  scale_y_continuous(labels = comma) +
  xlab("") +
  ylab("SEC Captures & Proprietary Validations vs  NDC PROPRIETARY Totals") +
  # scale_fill_manual(values = myCol6) +
...
## One Hit Wonder Portolio companies - 1 listing


```{r}
ggplot(doublebars_onendc, aes(x=reorder(family, n))) + #reorder(day, -perc)
  geom_bar(data = subset(doublebars_onendc, sec != "unval"), aes(y=-n, fill = sec), stat = 'identity') +
  geom_bar(data = subset(doublebars_onendc, sec != "ndc"), aes(y=n, fill = sec), stat = "identity") +
  geom_text(data = doublebars_perc_onehit, aes(reorder(family, ndc), y = -140), label = doublebars_perc_onehit$ndc_perc) + 
  geom_text(data = doublebars_perc_onehit, aes(reorder(family, ndc), y = +150), label = doublebars_perc_onehit$val_perc) + 
  scale_y_continuous(labels = comma) +
  xlab("") +
  ylab("SEC Captures & Proprietary Validations vs  NDC PROPRIETARY Totals") +
  # scale_fill_manual(values = myCol6) +
