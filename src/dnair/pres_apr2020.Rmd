...
```{r}
data_all_orig <- readRDS("/project/biocomplexity/sdad/projects_data/ncses/bi/binn/working/sec/wordlist_april21_2020_MANIP/sec_wordlists_captures_april212020.RDS")
# data_all <- data_all_orig %>% filter(nchar(Words) > 3 | str_detect(str_to_lower(Words), "new")) 
...
brand_words <- readRDS("/project/biocomplexity/sdad/projects_data/ncses/bi/binn/working/sec/sec_wordlists_apr21_2020_SYM/data_sym_march2020.RDS") %>% 
  select(-Capture, -Symbol_string) %>% 
  mutate(English = FALSE, Words = Word, Capitals = str_detect(Word, "[A-Z]"), id = row_number()) %>% 
  select(-Word) %>% 
...
```{r sizes}
sizes <- readRDS("~/orig_to_filtered_steps.RDS")

# sizes <- data_all_orig %>% count(Company, Year)
# print("original ingested word count")
# scales::comma(sum(sizes$n))
# 
# sizes_3new <- data_all_3NEW %>% count(Company, Year)
# print("original ingested word count - 3 letter words except new")
# scales::comma(sum(sizes_3new$n))

sizes %>% select(-Diff) %>% reshape2::dcast(Step~Method, fun.aggregate = sum)
sizes %>% select(-Set) %>% reshape2::dcast(Step~Method)

```



For 156 companies, there are 615 filings over 2012-2018 representing a total of 55,744,108. At the end, I'll subset for 2013-2017 (613 filings).
...
```{r}
company_years <- data_all_3NEW %>% count(Company, Year) %>% select(-n)

# nrow(company_years)
# table(company_years$Year)
```

```{r}
differences <- sizes %>% select(-Set) %>% reshape2::dcast(Step~Method)
sets <- sizes %>% select(-Diff) %>% reshape2::dcast(Step~Method)

sets <- sizes %>% filter(Step == "0. Original") %>% transmute(Method, Sum = Set) 

perc_diff <- sizes %>% left_join(sets, by = c("Method")) %>% mutate(Perc_diff = round(100*((-Diff)/Sum), digits = 2)) %>% select(1,2,6) %>% reshape2::dcast(Step~Method)
...
```{r}
orig_filing_size <- data_all_orig %>% count(Company, Year) %>% transmute(Company, Year, full_orig = n)
all3new_filing_size <- data_all_3NEW %>% count(Company, Year) %>% transmute(Company, Year, all3new = n)
launch_filing_size <- launch_proximity_words %>% count(Company, Year) %>% transmute(Company, Year, launch = n)
newprod_filing_size <-new_PROD_proximity_words %>% count(Company, Year) %>% transmute(Company, Year, newprod = n)
newdrug_filing_size <-new_DRUG_proximity_words %>% count(Company, Year) %>% transmute(Company, Year, newdrug = n)
newdevice_filing_size <- new_DEVICE_proximity_words %>% count(Company, Year) %>% transmute(Company, Year, newdevice = n)
brand_filing_size <- brand_words %>% count(Company, Year) %>% transmute(Company, Year, brand = n)

ingested_filing_size <- orig_filing_size %>% 
  left_join(all3new_filing_size, by = c("Company", "Year")) %>% 
  left_join(launch_filing_size, by = c("Company", "Year")) %>%
  left_join(newprod_filing_size, by = c("Company", "Year")) %>%
  left_join(newdrug_filing_size, by = c("Company", "Year")) %>%
  left_join(newdevice_filing_size, by = c("Company", "Year")) %>%
  left_join(brand_filing_size, by = c("Company", "Year")) 
...
```{r}
# orig_filing_size <- data_all_orig %>% count(Company, Year) %>% transmute(Company, Year, full_orig = n)
all3new_filing_size <- all_non_eng %>% count(Company, Year) %>% transmute(Company, Year, all3new = n)
launch_filing_size <- launch_prox %>% count(Company, Year) %>% transmute(Company, Year, launch = n)
newprod_filing_size <-newprod_prox %>% count(Company, Year) %>% transmute(Company, Year, newprod = n)
newdrug_filing_size <-newdrug_prox %>% count(Company, Year) %>% transmute(Company, Year, newdrug = n)
newdevice_filing_size <- newdevice_prox %>% count(Company, Year) %>% transmute(Company, Year, newdevice = n)
brand_filing_size <- brand_words %>% count(Company, Year) %>% transmute(Company, Year, brand = n)

filtered_filing_size <- all3new_filing_size %>% 
  left_join(launch_filing_size, by = c("Company", "Year")) %>%
  left_join(newprod_filing_size, by = c("Company", "Year")) %>%
  left_join(newdrug_filing_size, by = c("Company", "Year")) %>%
  left_join(newdevice_filing_size, by = c("Company", "Year")) %>%
