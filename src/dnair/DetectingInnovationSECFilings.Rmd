...
new_ref_companies <- tibble(company_reference_names$CompanyString, comp_tokens) %>% # head() %>% 
  tidyr::unnest(c(comp_tokens)) %>% filter(nchar(comp_tokens) >1) %>% as.data.frame() %>%
  ...
  mutate(comp_low_hun = corpus::text_tokens(comp_lowword, stemmer = stem_hunspell)) %>% tidyr::unnest(c(comp_low_hun)) %>% distinct()
