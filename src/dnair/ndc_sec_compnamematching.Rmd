...
ciknames %>% filter(str_detect(str_to_lower(company_names), pattern = "collegium"))
```
...
```{r}
cik_companies <- cik_2834_names %>% mutate(name = str_trim(str_to_lower(str_replace_all(company_names, "[:punct:]", replacement = " ")))) %>% distinct()
...
```{r}
ndc_companies %>% filter(str_detect(name, "genv"))
