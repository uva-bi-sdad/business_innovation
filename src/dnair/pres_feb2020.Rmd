...
```{r}
ndc_product %>% select(3,4,6:11, 13:14, 15:17) %>% head() %>% View()
...
ggplot(data = universe_ndc_listings_ct, aes(x = year, y = n)) +
  geom_bar(stat = "identity") +
  #geom_text(data = products_ct %>% filter(product == "listing") %>% filter(n > 1000),  stat = "identity", aes(label = n), just = 1.5, width = 1) +
  customPlot2 +
  coord_flip() +
...
```{r}
ggplot(data = labelers_over_years, aes(x = year, y = n )) +
  geom_bar(stat = "identity") +
...
ggplot(data = labelers_over_years %>% filter(year > 2012 ), aes(x = year, y = n )) +
  geom_bar(stat = "identity") +
  #geom_text(stat = "count", aes(label = ..count..), just = 1.5, width = 1) +
  customPlot2 +
  coord_flip() +
...
```{r}
ggplot(data = top10, aes(x = LABELERNAME, y = n, fill = top)) +
...
```{r ndc_manip}
universe_ndc_labelers <- ndc_product %>%
  select(year, PRODUCTTYPENAME, MARKETINGCATEGORYNAME, LABELERNAME) %>%
  distinct()

universe_ndc_prod_labelers <- ndc_product %>%
  #select(year, PROPRIETARYNAME, NONPROPRIETARYNAME, SUBSTANCENAME, PRODUCTTYPENAME, MARKETINGCATEGORYNAME, LABELERNAME) %>%
  select(year, LABELERNAME, PROPRIETARYNAME, NONPROPRIETARYNAME, SUBSTANCENAME) %>% # PRODUCTTYPENAME, MARKETINGCATEGORYNAME, formulationcode, 
  distinct()

ndc_product %>% filter(PROPRIETARYNAME == "Valcyte")
