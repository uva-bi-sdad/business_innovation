...
```{r}
sec_prop_bars <- prop_pos_matches_sec_ndc %>% count(LABELERNAME, prop_match) %>% select(-n) %>% count(LABELERNAME) %>% mutate(bucket = "sec_capture")
...
ciknames %>% filter(str_detect(str_to_lower(company_names), "endo p"))
ciknames %>% filter(cik == "1100962")

sic  %>% filter(str_detect(str_to_lower(Name), "endo ph"))

doublebars_1 <- doublebars_1 %>% left_join(ndc_sec_ref, by = c("LABELERNAME" = "NDCCompany")) 
doublebars_1 <- doublebars_1 %>% select(-SECCompany, -sic)

intersect(as.character(final_product_list_singlefirstmentionsONLY$Company), doublebars_1$cik)

totalcapturesdoublebars <- final_product_list_singlefirstmentionsONLY %>% 
  filter(as.character(Company) %in% doublebars_1$cik) %>% 
  select(Company, Token, Protect) %>% 
  count(Company, Token) %>% select(-n) %>%
  mutate(Company = as.character(Company)) %>%
  count(Company) %>% 
  left_join(ndc_sec_ref, by = c("Company" = "cik")) %>%
  select(LABELERNAME = NDCCompany, n) %>%
  mutate(bucket = "sec_unval_caps")
...
totalcapturesdoublebars <- final_product_list_singlefirstmentionsONLY_C %>%
  filter(Company_c %in% doublebars_1$cik) %>% 
  left_join(ndc_sec_ref, by = c("Company_c" = "cik")) %>% 
  count(NDCCompany, Token) %>%
  select(-n) %>% 
  count(NDCCompany)

# doublebars_2 <- doublebars_1 %>% 
#   left_join(totalcapturesdoublebars, by = c("cik" = "Company")) %>% 
#   group_by(LABELERNAME, num = n.x, bucket = recode(bucket, `sec_capture` = "sec_val_capts")) %>% 
#   summarise(num2 = sum(n.y, na.rm = T)) %>% filter(num2 >0)

rbind(doublebars_1 %>% select(-cik) %>% distinct(), totalcapturesdoublebars) %>% arrange(LABELERNAME)
