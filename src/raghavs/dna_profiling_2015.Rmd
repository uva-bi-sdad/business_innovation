...
dna.dt <- dna.dt %>%
  dplyr::select(c(an, title, body, publisher_name,
                  company_codes, company_codes_about, subject_codes, everything()))
```
...
large_articles <- subset(dna.2.dt, dna.2.dt$word_count > 4000)
#head(large_articles)
...
* Missing codes about and codes are nearly identical in each year (suggesting poor articles?)   
...
bc <- dna.3.dt[dna.3.dt$subject_codes_split %like% "c22", ]
