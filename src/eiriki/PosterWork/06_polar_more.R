# polar for more companies than before
# library
library(tidyverse)
library(viridis)
library(readxl)
library(dplyr)
data <- read.csv('./data/business_innovation/working/Polar_data/all_data.csv')
colnames(data) <- c("Company", "Trade Journal", "2013","2014","2015", "Total Count")
...
data = data %>% arrange(data$`Trade Journal`, data$`Total Count`)
cheat <- seq(1,nrow(data),by=1)
data$cheat <- cheat


# Get the name and the y position of each label
label_data= data %>% group_by(id, Company,`Trade Journal`) %>% summarize(tot=sum(Count))
label_data = label_data %>% arrange(label_data$`Trade Journal`, tot)
label_data$cheat <- seq(1,nrow(label_data),by=1)
number_of_bar=nrow(label_data)
angle= 90 - 360 * (label_data$cheat-0.5) /number_of_bar     # I substract 0.5 because the letter must have the angle of the center of the bars. Not extreme right(1) or extreme left (0)
label_data$hjust<-ifelse( angle < -90, 1, 0)
label_data$angle<-ifelse(angle < -90, angle+180, angle)
#manual fix
label_data$angle[43] <- (-39.33)
...
p = ggplot(data) +
  ...
  # Add the stacked bar
  geom_bar(aes(x=reorder(as.factor(id), as.numeric(data$cheat)), y=data$Count, fill=Year), stat="identity", alpha=0.5) +
  ...
  annotate("text", x = rep(max(data$id),4), y = c(0,10,20,30), label = c("0", "10", "20", "30") , color="black", size=5 , angle=0, fontface="bold", hjust=1) +
  ...
  geom_text(data=label_data, aes(x = (as.factor(id)), y=tot, label=paste(Company,paste0('(',tot,')'),by=' '), hjust=hjust),
